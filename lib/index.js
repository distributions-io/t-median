...
function median( v, options ) {
	...
	if ( isTypedArrayLike( v ) ) {
		...
		return median6( out, v );
	}
	if ( isArrayLike( v ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return median4( v, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return median3( out, v, opts.accessor );
		}
		return median2( out, v );
	}
	return NaN;
} // end FUNCTION median()
